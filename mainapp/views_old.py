...
from .models import Buyer, OwnerProfile
...
class BuyerProfileDetailView(DetailView):
    model = Buyer
    template_name = 'mainapp/buyer/buyer_profile.html'
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if OwnerProfile.objects.filter(pan_num=self.object.pk).exists():
            context["owner"] = OwnerProfile.objects.filter(pan_num=self.object.pk).values()[0]
...
class BuyerListView(ListView):
    model = Buyer
    template_name = 'mainapp/buyer_list.html'
    # context_object_name = 'buyers_with_owners'
    context_object_name = 'buyers'


    def get_queryset(self):
        return Buyer.objects.prefetch_related('ownerprofile_set').all()


class BuyerCreateView(CreateView):
    model = Buyer
    form_class = BuyerForm
    template_name = 'mainapp/buyer/add_buyer.html'
...
class BuyerUpdateView(UpdateView):
    ...
    template_name = 'mainapp/buyer/update_buyer.html'


class BuyerProfileUpdateView(UpdateView):
    model = OwnerProfile
    form_class = EditBuyerProfileForm
    template_name = 'mainapp/buyer/update_owner_info.html'
...
class BuyerDeleteView(DeleteView):
    model = Buyer
    template_name = 'mainapp/buyer/delete_buyer.html'
    success_url = reverse_lazy('buyer-list')
...
#     model = Farmer
#     template_name = 'mainapp/farmer_list.html'
